...
# the current directory
function installPkg2023()
{
    pacman -U --noconfirm $(ls | grep maya2023 | grep .zst)
...
function installLib2023()
{
    # Create Maya's lib folder
    mkdir -p /usr/autodesk/maya2023/lib

    # Copy libraries to Maya's lib folder
    cp -r $1/lib/libtiff/4.4.0-1/* /usr/autodesk/maya2023/lib/
    cp -r $1/lib/libjbig/2.1-23/* /usr/autodesk/maya2023/lib/
    cp -r $1/lib/libffi/3.1-3.1-32/* /usr/autodesk/maya2023/lib/

    # Link libraries to Maya's lib folder
    ln -s /usr/lib/libcrypt.so.2.0.0 /usr/autodesk/maya2023/lib/libcrypt.so.1

    # Create libmd.so into lib folder
    touch /usr/autodesk/maya2023/lib/libmd.so
}
...
function uninstallPkg2023()
{
    ...
    pacman -Rns --noconfirm $(pacman -Qm | grep maya2023 | awk '{print $1}')
}
